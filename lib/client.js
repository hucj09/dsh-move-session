...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function MoveSessionDialog(props) {
      ...
      if (dialog.done) {
          ...
          h(
            ...
            h('button', {
              ...
              onClick: function () {
                ...
                openSession(dialog.done.sessionId, function (error) {
                  setState({ open: true, busy: false, error: t('dialog.error.openFailed') + (error && error.message ? error.message : String(error)), done: null })
                })
              },
        ...
        body = h(
          ...
          targets.length === 0
            ...
            : h('div', { className: 'dsh-ms-options' }, targetRows),
      ...
      return h(
        ...
        h(
          ...
          h('div', { className: 'dsh-ms-head' }, h('div', { className: 'dsh-ms-title' }, t('dialog.title')), h('div', { className: 'dsh-ms-subtitle' }, sessionTitle)),
          body,
        ),
      )
    }
    ...
    /*   - a capture-phase click listener records the session row    */
    /*     whose ellipsis was clicked and recovers its session id    */
    /*     from the React fiber (node.id),                           */
    /*   - a MutationObserver watches for a newly opened             */
    /*     [role="menu"] containing the rename entry and appends a   */
    /*     menuitem that opens the move dialog.                      */
    ...
    /** Whether an open menu is the session-row menu (contains the rename entry). */
    ...
    function installRowMenuInjection() {
      var onDocumentClick = function (event) {
        ...
        var ellipsis = target.closest('button[aria-label^="会话\u201c"], button[aria-label^="Session \u201c"]')
        if (ellipsis === null) return
        var row = ellipsis.closest('[role="treeitem"]')
        if (row !== null) lastRowSessionId = sessionIdFromRow(row)
      }
      ...
      var observer = new MutationObserver(function (mutations) {
        for (var i = 0; i < mutations.length; i++) {
          ...
          for (var j = 0; j < added.length; j++) {
            ...
            if (isSessionRowMenu(menu)) injectMoveMenuItem(menu)
          }
        }
      })
    ...
    function apply(ctx) {
      ...
      if (typeof document !== 'undefined' && !document.head.querySelector('style[data-dsh-move-session]')) {
        ...
        style.textContent =
          ...
          '.dsh-ms-menu-item:hover{background:rgba(127,127,127,.12)}' +
          '.dsh-ms-menu-item svg{flex:none;opacity:.8}' +
          /* modal */
          '.dsh-ms-backdrop{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.45)}' +
          '.dsh-ms-card{width:480px;max-width:calc(100vw - 32px);max-height:calc(100vh - 64px);overflow:auto;padding:20px;border-radius:12px;background:var(--surface,#ffffff);color:var(--text-primary,#111827);box-shadow:0 12px 40px rgba(0,0,0,.28);font:14px/1.5 system-ui,-apple-system,"Segoe UI",sans-serif}' +
          '.dsh-ms-head{margin-bottom:14px}' +
          '.dsh-ms-title{font-size:16px;font-weight:600}' +
          '.dsh-ms-subtitle{margin-top:2px;font-size:13px;opacity:.65;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}' +
          '.dsh-ms-note{padding:8px 10px;margin-bottom:12px;border-radius:8px;background:rgba(127,127,127,.1);font-size:12.5px;opacity:.85}' +
          '.dsh-ms-field-label{margin:12px 0 6px;font-size:12.5px;font-weight:600;opacity:.8}' +
          '.dsh-ms-options{display:flex;flex-direction:column;gap:6px}' +
          '.dsh-ms-option,.dsh-ms-mode{display:flex;align-items:flex-start;gap:8px;padding:8px 10px;border:1px solid rgba(127,127,127,.25);border-radius:8px;cursor:pointer}' +
          '.dsh-ms-option-selected,.dsh-ms-mode-selected{border-color:var(--primary,#4f7cff);background:rgba(79,124,255,.08)}' +
          '.dsh-ms-option input,.dsh-ms-mode input{margin-top:3px;accent-color:var(--primary,#4f7cff)}' +
          '.dsh-ms-option-body{display:flex;flex-direction:column;min-width:0}' +
          '.dsh-ms-option-title{font-weight:500}' +
          '.dsh-ms-option-meta{font-size:12px;opacity:.6;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}' +
          '.dsh-ms-empty{padding:10px;border-radius:8px;border:1px dashed rgba(127,127,127,.35);text-align:center;font-size:13px;opacity:.7}' +
          '.dsh-ms-error{margin-top:10px;padding:8px 10px;border-radius:8px;background:rgba(220,38,38,.1);color:#dc2626;font-size:12.5px}' +
          '.dsh-ms-footer{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}' +
          '.dsh-ms-btn{padding:6px 14px;border:1px solid rgba(127,127,127,.3);border-radius:8px;background:transparent;color:inherit;font:inherit;cursor:pointer}' +
          '.dsh-ms-btn:hover:not(:disabled){background:rgba(127,127,127,.12)}' +
          '.dsh-ms-btn:disabled{opacity:.5;cursor:not-allowed}' +
          '.dsh-ms-btn-primary{border-color:var(--primary,#4f7cff);background:var(--primary,#4f7cff);color:#fff}' +
          '.dsh-ms-btn-primary:hover:not(:disabled){background:color-mix(in srgb,var(--primary,#4f7cff) 85%,#000)}' +
          '.dsh-ms-success-title{font-size:15px;font-weight:600;margin-bottom:4px}' +
          '.dsh-ms-success-detail{font-size:13px;opacity:.75}'
